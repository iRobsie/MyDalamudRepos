...
public class NotTooltipTweaks : SubTweakManager<NotTooltipTweaks.SubTweak> {
    ...
    public abstract class SubTweak : BaseTweak {
        ...
        protected static unsafe SeString GetTooltipString(StringArrayData* stringArrayData, NotTooltipTweaks.ItemTooltipField field) => GetTooltipString(stringArrayData, (int)field); 
        protected static unsafe SeString GetTooltipString(StringArrayData* stringArrayData, NotTooltipTweaks.ActionTooltipField field) => GetTooltipString(stringArrayData, (int)field); 
        ...
        protected static unsafe void SetTooltipString(StringArrayData* stringArrayData, NotTooltipTweaks.ItemTooltipField field, SeString seString) {
            try {
        ...
        protected static unsafe void SetTooltipString(StringArrayData* stringArrayData, NotTooltipTweaks.ActionTooltipField field, SeString seString) {
            try {
                if (stringArrayData->AtkArrayData.Size <= (int)field) {
                    throw new IndexOutOfRangeException($"Attempted to set Index#{(int)field} ({field}) but size is only {stringArrayData->AtkArrayData.Size}");
                }
    ...
    public override string Name => "Not Tooltip Tweaks";
    ...
    private unsafe IntPtr ActionTooltipDetour(AtkUnitBase* addon, void* a2, ulong a3) {
        ...
        try {
            foreach (var t in SubTweaks.Where(t => t.Enabled)) {
                if (t is not NotTooltipTweaks.SubTweak st) continue;
